        ...
        $.ajax({
            ...
            success: function (data, status, xhr) {
                ...
                $(data).each(function (i) {
                    ...
                    html +=
                        '<tr><td><i>GroupName: </i>' + this.groupName + '</td><td><input type="checkbox" id="checkboxG' + i + '"><input type="hidden" id="inputG' + i + '" value="' + this._id + '"></td></tr>';
                    numberOfItems = i;
                });
                html += '</table>';
    ...
    createUser.click(function () {
        ...
        for (let i = 0; i <= numberOfItems; i++) {
            ...
            if ($(checkboxId).is(":checked")) {
                ...
                groupListString.push(String($(inputId).val()));  
            }
        }
        ...
        groupList.forEach(function(item){
            ...
            $.ajax({
            type: 'PATCH',
            url: 'http://localhost:8000/api/groups/'+item,
            data: data,
            contentType: "application/json",
            success: function (data, status, xhr) {
                console.log('Status: ' + status);
